...
void CommandLineInterface::handleCombinedJSON()
{
	...
	for (string const& contractName: contracts)
	{
		...
		if (requests.count(g_strFunDebug) && m_compiler->compilationSuccessful())
			contractData[g_strFunDebug] = StandardCompiler::formatFunctionDebugInfo(
				m_compiler->object(contractName).functionDebugInfo
			);
		if (requests.count(g_strFunDebugRuntime) && m_compiler->compilationSuccessful())
			contractData[g_strFunDebugRuntime] = StandardCompiler::formatFunctionDebugInfo(
				m_compiler->runtimeObject(contractName).functionDebugInfo
			);
