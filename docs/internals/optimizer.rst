...
generated bytecode and the Yul optimizer for the Yul code generated for ABIv2 codec.
For a Yul-based codegen, one can use ``solc --ir-optimized --optimize`` to produce
optimized Yul for a Solidity source. Similarly, use ``solc --strict-assembly --optimize``
